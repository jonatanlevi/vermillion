...
function GlassButton({ onPress, t }) {
  ...
  return (
    <View style={gb.wrap}>
      <Text style={gb.label}>{t.glassLockLabel}</Text>
      <Text style={gb.sub}>{t.glassLockSub}</Text>
      <View style={gb.scene}>
...
function CommitModal({ visible, time, onClose, t }) {
    ...
    <Modal visible={visible} transparent animationType="none">
      <View style={cm.overlay}>
        <Animated.View style={[cm.card, { opacity: opacAnim, transform: [{ scale: scaleAnim }] }]}>
          ...
          <Text style={cm.title}>{t.commitModalTitle}</Text>
          <Text style={cm.time}>{h}:{m}</Text>
          <Text style={cm.desc}>{t.commitModalDesc(h, m)}</Text>
          <Text style={cm.note}>{t.commitModalNote}</Text>
          <TouchableOpacity style={cm.btn} onPress={onClose} activeOpacity={0.85}>
            <Text style={cm.btnText}>{t.commitModalBtn}</Text>
          </TouchableOpacity>
        </Animated.View>
      </View>
    </Modal>
...
export default function GamesScreen({ navigation }) {
  ...
  const GAMES = [
    { key: 'runner',   label: t.gamesRunnerLabel,   emoji: '🏃', desc: t.gamesRunnerDesc,   color: '#C0392B' },
    { key: 'breakout', label: t.gamesBreakoutLabel,  emoji: '🧱', desc: t.gamesBreakoutDesc,  color: '#E67E22' },
    { key: 'obstacle', label: t.gamesObstacleLabel,  emoji: '🐦', desc: t.gamesObstacleDesc,  color: '#8E44AD' },
  ];
    ...
    return (
      <Animated.View style={[styles.gameFullscreen, { opacity: fadeAnim, paddingTop: insets.top }]}>
        <TouchableOpacity style={styles.exitBtn} onPress={() => setActiveGame(null)}>
          <Text style={styles.exitBtnText}>{t.gamesExit}</Text>
        </TouchableOpacity>
  ...
  if (showDailyStamp) {
    ...
    const accuracyLabel =
      ...
      accuracySec < 30     ? t.stampAcc1(accuracySec) :
      accuracySec < 120    ? t.stampAcc2(accuracySec) :
      t.stampAcc3(Math.round(accuracySec / 60));

    return (
      <View style={[styles.commitScreen, { paddingTop: insets.top + 20 }]}>
        <Text style={ds.dayLabel}>{t.stampDayLabel(activeDay)}</Text>
        <Text style={ds.title}>{t.stampTitle}</Text>
        <Text style={ds.commitTimeDisplay}>{h}:{m}</Text>
        {stampAccuracy === null ? (
          <>
            <Text style={ds.sub}>{t.stampSub}</Text>
            <TouchableOpacity style={ds.stampBtn} onPress={handleDailyStamp} activeOpacity={0.85}>
              <Text style={ds.stampBtnText}>{t.stampBtn}</Text>
            </TouchableOpacity>
          </>
        ) : (
          <>
            ...
            <TouchableOpacity style={ds.continueBtn} onPress={handleStampDone} activeOpacity={0.85}>
              <Text style={ds.continueBtnText}>{t.stampContinue}</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
    ...
    return (
      <View style={[styles.commitScreen, { paddingTop: insets.top + 20 }]}>
        <View style={styles.scoreResult}>
          <Text style={styles.scoreResultLabel}>{t.gamesGameDone}</Text>
          <Text style={styles.scoreResultVal}>+{sessionScore} נקודות 🔥</Text>
        </View>
        <GlassButton onPress={handleCommit} t={t} />
      </View>
    );
  }
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.daysLeftBadge}>
            <Text style={styles.daysLeftText}>{t.gamesDaysLeft(MONTH_DAYS_LEFT)}</Text>
          </View>
        </View>
        ...
        <View style={styles.scoreCard}>
          <View style={styles.scoreItem}>
            ...
            <Text style={styles.scoreLabel}>{t.gamesRankLabel}</Text>
          </View>
          <View style={styles.scoreDivider} />
          <View style={styles.scoreItem}>
            <Text style={styles.scoreValue}>{totalScore.toLocaleString()}</Text>
            <Text style={styles.scoreLabel}>{t.gamesScoreLabel}</Text>
          </View>
          <View style={styles.scoreDivider} />
          <View style={styles.scoreItem}>
            <Text style={styles.scoreValue}>×1.2</Text>
            <Text style={styles.scoreLabel}>{t.gamesMultiplierLabel}</Text>
          </View>
        </View>

        {sessionScore > 0 && (
          <View style={styles.sessionBanner}>
            <Text style={styles.sessionText}>{t.gamesSessionBanner(sessionScore)}</Text>
          </View>
        )}

        <Text style={styles.sectionTitle}>{t.gamesChooseGame}</Text>
        ...
        <View style={styles.prizeCard}>
          <Text style={styles.prizeTitle}>{t.gamesPrizeTitle}</Text>
          <Text style={styles.prizeAmount}>{t.gamesPrizeAmt}</Text>
          <Text style={styles.prizeSub}>{t.gamesPrizeSub}</Text>
        </View>
      </ScrollView>

      <CommitModal visible={showModal} time={commitTime} onClose={() => setShowModal(false)} t={t} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  scoreCard: { flexDirection: 'row', backgroundColor: '#111', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#1E1E1E', justifyContent: 'space-around', alignItems: 'center' },
  ...
  sessionBanner: { backgroundColor: '#0D1A0D', borderRadius: 10, padding: 10, marginBottom: 16, borderWidth: 1, borderColor: '#1A3A1A', alignItems: 'center' },
  ...
  gameCard: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: '#111', borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1 },
  ...
  prizeCard: { backgroundColor: '#1A1400', borderRadius: 16, padding: 20, marginTop: 8, borderWidth: 1, borderColor: '#D4AF3744', alignItems: 'center' },
  ...
  commitScreen: { flex: 1, backgroundColor: '#0A0A0A', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24 },
...
const ds = StyleSheet.create({
  ...
  stampBtn: { backgroundColor: '#C0392B', borderRadius: 16, paddingVertical: 20, paddingHorizontal: 48, alignItems: 'center' },
  ...
  continueBtn: { backgroundColor: '#1A1A1A', borderRadius: 16, borderWidth: 1, borderColor: '#C0392B44', paddingVertical: 18, paddingHorizontal: 32, alignItems: 'center', width: '100%' },
...
const gb = StyleSheet.create({
  ...
  glow:  { position: 'absolute', width: 160, height: 160, borderRadius: 80, backgroundColor: '#D4AF37' },
  dome:  { position: 'absolute', top: 0, width: 160, height: 130, borderTopLeftRadius: 80, borderTopRightRadius: 80, backgroundColor: 'rgba(255,255,255,0.12)', borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', zIndex: 2 },
  domeInner: { width: 40, height: 60, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.07)', position: 'absolute', top: 12, right: 20 },
  base:  { position: 'absolute', bottom: 0, width: 160, height: 120, backgroundColor: '#2A2218', borderRadius: 12, borderWidth: 2, borderColor: '#3D3020', alignItems: 'center', justifyContent: 'center', zIndex: 1 },
  btnTouch: { zIndex: 3 },
  btn:  { width: 72, height: 72, borderRadius: 16, backgroundColor: '#D4AF37', alignItems: 'center', justifyContent: 'center', shadowColor: '#D4AF37', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 12, elevation: 12, borderWidth: 2, borderColor: '#F5D060' },
  btnText: { color: '#1A0A00', fontSize: 13, fontWeight: '900', letterSpacing: 1 },
  lever:    { position: 'absolute', right: 16, bottom: 20, transformOrigin: 'bottom', zIndex: 4 },
  ...
  bolt: { position: 'absolute', width: 8, height: 8, borderRadius: 4, backgroundColor: '#4A3A28', borderWidth: 1, borderColor: '#5A4A38' },
});

const cm = StyleSheet.create({
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.88)', alignItems: 'center', justifyContent: 'center', padding: 32 },
  card:    { backgroundColor: '#111', borderRadius: 24, padding: 32, alignItems: 'center', borderWidth: 1, borderColor: '#D4AF3744', width: '100%' },
